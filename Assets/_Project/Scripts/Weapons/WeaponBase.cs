...
public abstract class WeaponBase : MonoBehaviour
{
    ...
    [SerializeField] protected LayerMask _targetableLayerMask;
    ...
    protected virtual void Shoot()
    {
        ...
        if (Physics.Raycast(_bulletSpawnPosition.position, shootDirection, out RaycastHit hit, _targetableLayerMask))
        {
            StartCoroutine(SpawnTrail(trail, hit.point, hit.normal, true));
        }
        else
        {
            StartCoroutine(SpawnTrail(trail, _bulletSpawnPosition.position + shootDirection * _missDistance, Vector3.zero,
                false));
        }
    }

    protected IEnumerator SpawnTrail(TrailRenderer trail, Vector3 hitPoint, Vector3 hitNormal, bool impact)
    {
